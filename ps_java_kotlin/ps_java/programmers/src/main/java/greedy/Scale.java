...
public class Scale {
     /*   public static int solution(int[] weight) {
        ...
        while(true) {
            ...
            if(target >= weight[weight.length-1])
                ...
                floorIndex = binaryFloor(weight, 0, weight.length/2, weight.length,
                answer, weight.length-1);
    ...
    private static int binaryFloor(int[] weight, int start, int mid, int end, int key,
    int max) {
        if (start > max)
