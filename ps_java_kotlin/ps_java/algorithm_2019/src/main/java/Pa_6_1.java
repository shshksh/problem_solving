...
public class Pa_6_1 {
    ...
    private static void initDictionary(RBTree wordTree) throws NoSuchElementException,
            FileNotFoundException {
        String path = System.getProperty("user.dir");
        path += "\\algorithm\\src\\algorithm.algorithm_2019\\resource\\shuffled_dict.txt";
    ...
    static class RBTree {
        ...
        private void deleteFixUp(WordNode x) {
            while (x != root && x.color.equals("black")) {
                if (x == x.superNode.leftNode) {
                    ...
                    if (w.leftNode.color.equals("black") && w.rightNode.color.equals(
                            "black")) {
                        w.color = "red";
                        x = x.superNode;
                    } else {
                    ...
                    if (w.rightNode.color.equals("black") && w.leftNode.color.equals(
                            "black")) {
                        w.color = "red";
                        x = x.superNode;
                    } else {
        ...
        public void deleteAll(String fileName) throws FileNotFoundException {
            String path = System.getProperty("user.dir");
            path += "\\algorithm\\src\\algorithm.algorithm_2019\\resource\\";
            path += fileName;
            Scanner fileScanner = new Scanner(new File(path));
            int cnt = 0;
            String word;
