...
public class AreaFile {
    ...
    private void addToMap() throws FileNotFoundException {
        String path = "algorithm/src/algorithm.algorithm_2020/res/" + "alabama.txt";
        ...
        while (fileScanner.hasNextLine()) {
            ...
            AreaMap.put(line[0], new Area(line[0], Double.parseDouble(line[1]),
                    Double.parseDouble(line[2])));
        }
    }

    private void connectEdges() throws CloneNotSupportedException, FileNotFoundException {
        String path = "algorithm/src/algorithm.algorithm_2020/res/" + "roadList2.txt";
        File file = new File(path);
    ...
    private void connect(Area header, Area target) {
        ...
        header.next = target;
        target.weigh = calDistance(header.latitude, header.longitude, target.latitude,
                target.longitude);
    }
    ...
    private void offerAllAdjacentLocationOf(Queue<Area> q, Area target, HashMap<String,
            Boolean> hashChk) {
        while (target != null) {
    ...
    private HashMap<String, Area> shortestPathOf(Area src) {
        ...
        PriorityQueue<Area> q =
                new PriorityQueue<>(Comparator.comparingDouble(o -> o.weigh));
    ...
    private void initialize(HashMap<String, Area> chosen, PriorityQueue<Area> q,
                            Area src) {
    ...
    private void updateWeigh(HashMap<String, Area> chosen, PriorityQueue<Area> q,
                             Area added) {
