...
public class Pa_4_1 {
    ...
    private static void calcAllSortFor(int[] data, double[] time) {
        ...
        num = 0;
        time[4] += quickSort(Arrays.copyOf(data, data.length), 0, data.length - 1, 1);
        // pivot: last element
        num = 0;
        time[5] += quickSort(Arrays.copyOf(data, data.length), 0, data.length - 1, 2);
        // pivot: medium element
        num = 0;
        time[6] += quickSort(Arrays.copyOf(data, data.length), 0, data.length - 1, 3);
